...
- **Audio**: "We evaluated five diverse models: GLM 5.2, MiMo V2.5, Gemma 4 31B, Kimi K2.6, and Llama 3.3 70B. Each model was evaluated on both pt-BR prompts and matched English baselines. Kimi K2.6 needed a higher visible-output budget because some runs spent tokens on reasoning-style output before the final answer. To validate our labels, we used a multi-label classification rubric and performed a delayed blind manual audit on a subset of responses."
...
- **Audio**: "Here is our main result. On the X-axis, we have the harmful compliance rate, and on the Y-axis, we have the over-refusal rate. The lower-left corner is the ideal safe zone. Over-refusal was zero across models, but unsafe compliance and safe-redirect behavior varied substantially. Kimi K2.6 refused all harmful prompts, while Llama 3.3 70B had the highest harmful compliance and never redirected victims to official help. We also found that an early, larger cross-language gap was mostly a scoring artifact caused by English contraction and apostrophe handling."

## [2:00 - 2:30] Implications & Gating Policy
- **Visual**: GitHub repository screen or taxonomy diagram. Public release policy slide.
- **Audio**: "These results demonstrate that cultural and linguistic context matters deeply for AI safety. PixTrap is designed to be low-cost, reproducible, and easy to resume. Because these prompts represent real-world social-engineering risks, we gate the raw harmful dataset to prevent misuse, while publishing the code, scoring heuristics, aggregate results, charts, sanitized harmful samples, and full benign prompts. We hope this methodology serves as a reusable template for other local AI safety benchmarks worldwide. Thank you."
